...
The State Qualifying Times chart (`app/components/StateTimesApp.tsx`, `lib/state-qualifying-times.mjs`) has its own five-color series-identity palette, chosen and validated the same way as a poster palette — a chart series color is not a website theme token either. Site maroon and gold/aqua are reused for the two most common standards (`#a33a5e`, a lightened maroon, and the exact `--gold`/`--aqua`); a blue `#4a6fb0` and an olive `#5a7a2a` were added because the site has no fourth or fifth brand accent and a swimmer's age can put up to four standards on screen at once. Colors are assigned by standard identity, fixed regardless of which subset a given age shows (`STANDARD_META` order in the lib file). Run `node scripts/validate_palette.js "#a33a5e,#c9963a,#4db6a0,#4a6fb0,#5a7a2a" --mode light --pairs all` from the dataviz skill's directory before touching these five — light mode only, since `app/layout.tsx` fixes `colorScheme: "light"` for the whole site.
