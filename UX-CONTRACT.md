...
When an imported meet is open, the walk-in panel reuses that meet's imported job list and credit defaults. The operator selects a swimmer, enters the volunteer's name, and may overwrite the imported hours before checking in. The saved walk-in is an assigned signup row, so it appears in the same on-deck/history views and the downloaded workbook; corrected uploads carry those walk-ins forward for the same meet.
