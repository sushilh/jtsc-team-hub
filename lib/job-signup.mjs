...
export function usableVolunteerInfo(value) {
  ...
  const candidate = first
    ...
    .replace(/^[\s,;|/-]+|[\s,;|/-]+$/g, "")
    .trim();
  if (!/[a-z]/i.test(candidate) || /\d/.test(candidate) || candidate.includes("@")) return "";
  return candidate;
}
...
export function parseSignupWorkbook(input) {
  ...
  const sourceRows = grid.slice(1).filter((row) => row.some((value) => text(value))).map((row, index) => {
    ...
    return {
      ...
      notes: get("Notes"),
      volunteerInfo,
      volunteerName: usableVolunteerInfo(volunteerInfo),
    };
  });
  // TeamUnify sometimes supplies a volunteer name on only one of several shifts for
  // the same account. Reuse it only when that exact account has one unambiguous name.
  const namesByAccount = new Map();
  for (const row of sourceRows) {
    if (!row.account || !row.volunteerName) continue;
    const names = namesByAccount.get(row.account) ?? new Set();
    names.add(row.volunteerName);
    namesByAccount.set(row.account, names);
  }
  const rows = sourceRows.map((row) => {
    const accountNames = namesByAccount.get(row.account);
    const preferredName = accountNames?.size === 1 ? [...accountNames][0] : "";
    const volunteerName = row.volunteerName || preferredName || row.accountName;
    return { ...row, volunteerName, assigned: Boolean(row.accountName || volunteerName) };
  });
