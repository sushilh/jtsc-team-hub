...
export function normalizeCardEvents(events, eventName = "", time = "") {
  return (Array.isArray(events) ? events : [{ eventName, time }])
    .map(event => ({ eventName: String(event.eventName ?? "").trim(), time: String(event.time ?? "").trim() }))
    .filter(event => event.eventName || event.time);
}

export function createCaptions({ name = "", headline = "", eventName = "", time = "", events, subline = "", meetName = "", meetDate = "" } = {}) {
  const clean = value => String(value).trim();
  const swimmer = clean(name);
  const achievement = clean(headline);
  const event = normalizeCardEvents(events, eventName, time).map(row => [row.eventName, row.time].filter(Boolean).join(" · ")).join("\n");
