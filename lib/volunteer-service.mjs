...
async function handleCheckin(request, database, bindings) {
  ...
  if (body.action === "signup_checkin") {
    ...
    const row = await database.prepare(`SELECT credit_possible AS creditPossible, event_date AS eventDate
      FROM signup_rows WHERE id = ? AND active = 1 AND assigned = 1`).bind(id).first();
    if (!row) return fail("That volunteer assignment is no longer available.", 404);
    if (!isEarlyCheckinAllowed(bindings) && row.eventDate !== clubIsoDate()) {
