...
async function publicData(database) {
  const [sessions, jobs, swimmers, activeEntries, signupRows, signupDates] = await Promise.all([
...
async function adminData(database) {
...
async function importSignup(request, database) {
  ...
  if (eventTitles.length) {
    await database.prepare(`UPDATE signup_rows SET active = 0 WHERE active = 1 AND event_title IN (${eventTitles.map(() => "?").join(",")})`)
      .bind(...eventTitles).run();
  }
  const statements = parsed.rows.map((row) => database.prepare(`INSERT INTO signup_rows (
    ...
    .bind(
      ...
      row.account, row.accountName, row.swimmerName, row.jobName, row.creditPossible, row.creditEarned,
      row.creditUnits, row.slot, row.completed ? 1 : 0, row.notes, row.volunteerInfo, row.volunteerName,
      row.completed ? row.creditEarned : row.creditPossible,
    ));
  for (let index = 0; index < statements.length; index += 75) await database.batch(statements.slice(index, index + 75));
  return responseJson({
    ok: true,
    duplicate: false,
    import: { id: importId, fileName: file.name, rowCount: parsed.rows.length, eventCount: eventTitles.length, assignedCount },
  });
}
...
async function handleAdminMutation(request, database) {
  const body = await request.json();
...
export async function handleVolunteerRequest(request, bindings = env) {
  ...
  try {
    ...
    if (url.pathname === "/api/admin/login" && request.method === "POST") {
      const body = await request.json();
      if (String(body.pin ?? "") !== String(bindings.ADMIN_PIN || "216048")) return fail("That access code is not correct.", 401);
    ...
    if (url.pathname === "/api/admin" && request.method === "GET") return responseJson(await adminData(database));
    if (url.pathname === "/api/admin" && request.method === "POST") return await handleAdminMutation(request, database);
