...
const schemaStatements = [
  ...
  // A saved copy of a generated hour-ledger CSV, kept as a standing record independent
  // of the live roster. Not cleared by the season reset, unlike every other table here.
  `CREATE TABLE IF NOT EXISTS ledger_exports (
    id INTEGER PRIMARY KEY AUTOINCREMENT, label TEXT NOT NULL, file_name TEXT NOT NULL,
    row_count INTEGER NOT NULL, total_hours REAL NOT NULL, total_points REAL NOT NULL,
    csv_content TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
  )`,
...
async function publicData(database, bindings) {
  const [sessions, jobs, swimmers, activeEntries, signupRows, signupDates, unassignedMeets, signupJobs, signupSwimmers, signupMeetSettings] = await Promise.all([
    ...
    all(database, `SELECT event_date AS eventDate, event_title AS eventTitle, COUNT(*) AS openCount
      ...
      GROUP BY event_date, event_title ORDER BY event_date DESC, event_title`),
    all(database, `SELECT MIN(id) AS sourceId, event_date AS eventDate, event_title AS eventTitle,
      event_start AS eventStart, event_end AS eventEnd, job_name AS jobName,
      credit_possible AS creditPossible, credit_units AS creditUnits,
      COUNT(*) AS slotCount
      FROM signup_rows WHERE active = 1 AND job_name <> ''
      GROUP BY event_date, event_title, event_start, event_end, job_name, credit_possible, credit_units
...
async function adminData(database, bindings) {
  ...
  const [jobs, entries, signupImports, ledgerExports] = await Promise.all([
    all(database, `SELECT id, session_id AS sessionId, name, start_time AS startTime,
    ...
    all(database, `SELECT id, label, file_name AS fileName, row_count AS rowCount,
      total_hours AS totalHours, total_points AS totalPoints, created_at AS createdAt
      FROM ledger_exports ORDER BY created_at DESC, id DESC LIMIT 50`),
  ]);
  return {
    ...publicResult,
    resetAllowed,
    ...
    ledgerExports: ledgerExports.map((item) => ({
      ...item,
      id: Number(item.id), rowCount: Number(item.rowCount),
      totalHours: Number(item.totalHours), totalPoints: Number(item.totalPoints),
    })),
  };
}
...
async function importSignup(request, database) {
  ...
  if (existing) {
    ...
    // A live import may have extra walk-ins. Never treat a count mismatch on a
    // live roster as a failed upload and delete its assignments or attendance.
    if (activeRows > 0) {
  ...
  const live = await all(database, `SELECT event_date AS eventDate, event_title AS eventTitle,
      ...
      FROM signup_rows WHERE active = 1 AND (checked_in_at IS NOT NULL OR slot LIKE 'WALK-IN-%')`);
...
async function downloadLedgerExport(request, database) {
  const id = number(new URL(request.url).searchParams.get("id"), 1, Number.MAX_SAFE_INTEGER);
  if (!id) return fail("Choose a saved export to download.");
  const row = await database.prepare("SELECT file_name AS fileName, csv_content AS csvContent FROM ledger_exports WHERE id = ?").bind(id).first();
  if (!row) return fail("That saved export was not found.", 404);
  return new Response(row.csvContent, {
    headers: {
...
async function handleCheckin(request, database, bindings) {
  ...
  if (body.action === "signup_walkin") {
    ...
    const sourceId = body.sourceId == null ? null : number(body.sourceId, 1, Number.MAX_SAFE_INTEGER);
    if (body.sourceId != null && !sourceId) return fail("Choose an available job.");
    ...
    const source = await database.prepare(`SELECT sr.id, sr.import_id AS importId,
      ...
      ORDER BY sr.id LIMIT 1`).bind(eventDate, eventTitle, jobName, sourceId, sourceId).first();
    ...
    const insertWalkin = database.prepare(`INSERT INTO signup_rows (
      import_id, source_row, active, assigned, event_title, event_start, event_end, event_date,
      account, account_name, swimmer_name, job_name, credit_possible, credit_earned, credit_units,
      slot, completed, notes, volunteer_info, volunteer_name, credited_value,
      checked_in_at, checked_out_at, volunteer_override
    ...
    // Keep import metadata in step with appended rows in the same transaction.
    // A repeat upload must remain a no-op after a walk-in has been added.
    const [inserted] = await database.batch([
      insertWalkin,
      database.prepare(`UPDATE signup_imports SET
        row_count = (SELECT COUNT(*) FROM signup_rows WHERE import_id = ?),
        assigned_count = (SELECT COUNT(*) FROM signup_rows WHERE import_id = ? AND assigned = 1)
        WHERE id = ?`).bind(source.importId, source.importId, source.importId),
    ]);
...
async function handleAdminMutation(request, database, bindings) {
  ...
  if (body.action === "save_ledger_export") {
    const label = requiredText(body.label, "Meet label", 240);
    const fileName = requiredText(body.fileName, "File name", 200);
    const csv = requiredText(body.csv, "Export content", 2_000_000);
    const rowCount = number(body.rowCount, 0, 100_000);
    const totalHours = number(body.totalHours, 0, 1_000_000);
    const totalPoints = number(body.totalPoints, 0, 1_000_000);
    if (rowCount === null || totalHours === null || totalPoints === null) return fail("The export could not be saved.");
    const result = await database.prepare(`INSERT INTO ledger_exports
      (label, file_name, row_count, total_hours, total_points, csv_content)
      VALUES (?, ?, ?, ?, ?, ?)`).bind(label, fileName, rowCount, totalHours, totalPoints, csv).run();
    return responseJson({ ok: true, id: result.meta?.last_row_id });
  }
...
export async function handleVolunteerRequest(request, bindings) {
  ...
  try {
    ...
    if (url.pathname === "/api/admin/ledger-export" && request.method === "GET") return await downloadLedgerExport(request, database);
